...
def _execute_list_tasks(db: Session, user_id: str, arguments: dict) -> list[dict]:
    tasks = task_service.list_tasks(db, user_id, done=arguments.get("done"))
...
def _execute_mark_task_done(db: Session, user_id: str, arguments: dict) -> dict:
    task_id = arguments.get("task_id")
    if task_id is None:
        return {"error": "Could not find a task id in your message. Please include a task id, e.g. 'mark task 1 as done'."}

    task = task_service.mark_task_done(db, user_id, task_id)
    if task is None:
...
def _execute_update_task(db: Session, user_id: str, arguments: dict) -> dict:
    task_id = arguments.get("task_id")
    if task_id is None:
...
def _execute_delete_task(db: Session, user_id: str, arguments: dict) -> dict:
    task_id = arguments.get("task_id")
    if task_id is None:
...
def execute_tool(decision: ToolDecision, db: Session, user_id: str) -> dict | list | None:
    """Run the tool selected by a decision, if execution is supported.

    user_id always comes from the authenticated request (see
    routes/agent.py), never from decision.arguments - the decision
    providers/tool schemas have no user_id field, so there is no channel
    for a client message to influence whose data gets touched.
    ...
    if selected_tool == "create_task":
        return _execute_create_task(db, user_id, arguments)

    if selected_tool == "list_tasks":
        return _execute_list_tasks(db, user_id, arguments)

    if selected_tool == "get_weather":
        return _execute_get_weather(arguments)

    if selected_tool == "mark_task_done":
        return _execute_mark_task_done(db, user_id, arguments)

    if selected_tool == "update_task":
        return _execute_update_task(db, user_id, arguments)

    if selected_tool == "delete_task":
        return _execute_delete_task(db, user_id, arguments)
